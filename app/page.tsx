...
export default function NeuroScanPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col selection:bg-cyan-500 selection:text-white">
      ...
      <ModelStatusBar status={modelStatus} onRetry={initModel} onOpenModelGuide={() => setIsInfoModalOpen(true)} />
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-200 pb-5">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl sm:text-2xl font-black tracking-tight text-slate-900">Brain MRI Neural Diagnostic Suite</h2>
            </div>
            <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-2xl font-normal">
              Real-time classification across 4 pathology tiers (Glioma, Meningioma, Pituitary, Normal). Powered by browser-native
              WebGL tensor execution with strict zero-upload privacy.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-5">
            {activeImageSrc ? (
              ...
              <Dropzone onImageSelected={handleImageSelected} disabled={isAnalyzing} />
            )}

            {/* Test Presets Carousel */}
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <SamplePresets onSelectPreset={handleSelectPreset} selectedPresetId={selectedPresetId} disabled={isAnalyzing} />
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-5">
            {inferenceResult ? (
              <>
                ...
                <PrimaryDiagnosis result={inferenceResult} onOpenReport={() => setIsReportModalOpen(true)} />

                {/* 4-Class Softmax Probability Distribution */}
                <ClassProbabilityBars probabilities={inferenceResult.probabilities} />

                {/* Performance & Tensor Memory Telemetry */}
                <BenchmarkMetrics result={inferenceResult} activeTensors={activeTensors} />
              </>
            ) : (
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
                ...
                <h3 className="text-base font-bold text-slate-800">Diagnostic Telemetry Standby</h3>
                ...
                <div className="mt-6 flex items-start gap-2 rounded-xl border border-slate-100 bg-slate-50 p-3 text-left text-[11px] text-slate-500">
                  ...
                  <span>
                    Designed for engineering evaluation and research. Always verify medical imaging results with a certified
                    radiologist.
                  </span>
                </div>
              </div>
      ...
      {/* Modals */}
      <ModelInfoModal isOpen={isInfoModalOpen} onClose={() => setIsInfoModalOpen(false)} onRefresh={initModel} />
